...
export async function GET(
  ...
  if (!dues) return NextResponse.json({ error: "Not found" }, { status: 404 });
  ...
  if (!dues.invoiceNumber) {
    const invoiceNumber = await nextInvoiceNumber(dues.clubId, dues.fiscalYear);
    await prisma.memberDues.update({
      where: { id },
      data: { invoiceNumber },
    });
    dues.invoiceNumber = invoiceNumber;
  }

  const { buffer, filename } = await buildDuesInvoicePdfBuffer(
    dues.club,
    dues.member,
    dues,
    locale
