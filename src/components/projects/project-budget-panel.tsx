...
export function ProjectBudgetPanel({
  ...
  function saveBudget() {
    startTransition(async () => {
      ...
      const result = await updateProjectBudget(projectId, {
  ...
  function addEntry() {
    ...
    startTransition(async () => {
      const result = await createProjectBudgetEntry(projectId, {
        type: entryForm.type,
        amount,
        date: entryForm.date,
    ...
    <Card>
      ...
      <CardContent className="space-y-6">
        ...
        <div className="space-y-3">
          ...
          {canManage && (
            <div className="grid sm:grid-cols-2 gap-2 p-3 rounded-xl border border-dashed border-gray-200 bg-white">
              <select
                className="h-10 rounded-lg border border-gray-200 px-2 text-sm"
                value={docForm.kind}
                onChange={(e) =>
